...
*/
  int revision = 5;
  uint64_t tsf_now=0;
  printf("TSF = 0x%016llx (%llu) [BEFORE]\n",tsf,tsf);
  if (revision >= 3) {
    uint32_t lo = (uint32_t)(tsf & 0x00000000FFFFFFFFULL);
    uint32_t hi = (uint32_t)((tsf & 0xFFFFFFFF00000000ULL) >> 32);
    printf("lo  = 0x%016x (%u)\n",lo,lo);
    printf("hi  = 0x%016x (%u)\n",hi,hi);

//    if (hi != 0x0) //WORKAROUND
  ...
  getTSFRegs(df, &tsf);
  printf("AFTER TSF = %llu\n",tsf);
  printf("TSF = 0x%016llx (%llu) [AFTER]\n",tsf,tsf);
}
