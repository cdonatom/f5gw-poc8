...
int main(int argc, char * argv[])
{
	...
	switch(current_options.OP_MODE){
		...
		case 0: 
			...
			if(!strcmp(current_options.reg_share,"5")){
				uint64_t tsf;
				uint64_t tsf_start=0;
				uint64_t tsf_=0;

				int sample = 0;
				uint64_t last_tsf = 0;
				uint64_t t_sleep=10e6;
				uint64_t DD,dd=0;
				uint8_t time_marker=0;
				getTSFRegs(&df,&tsf_start);
				tsf_=tsf_start;
				do {
					usleep(t_sleep);
					getTSFRegs(&df,&tsf);
			...
			if(!strcmp(current_options.reg_share,"6")){
					uint64_t tsf_now=0;
					uint64_t tsf= strtoull(current_options.set_tsf, NULL, 10);
					printf("tsf=%llu\n",tsf);
					getTSFRegs(&df,&tsf_now);
					tsf+=tsf_now;
			...
			{
				unsigned int gpr_drift_debug = shmRead16(&df,B43_SHM_REGS, 58);
				int tsf_int = atoi(current_options.set_tsf);
				printf("tsf_int = %d\n",tsf_int);
				shmWrite16(&df, B43_SHM_REGS,63, tsf_int);
				printf("gpr_debug = %d\n",gpr_drift_debug);
/*
				uint64_t tsf=0;
				uint64_t tsf_=0;
