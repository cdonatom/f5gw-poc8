...
//
	...
	do_not_erase_shm:;
		...
		;//WMP LTE variables INIT 
		mov	REGION_DEBUG_START,	STATE_DEBUG_04;
		mov 	STATE_DEBUG_04, STATE_DEBUG_02;
...
//
	...
	rx_fifofull:;
		...
		;//wait next slot (1ms slot)

			
				jnzx	0, 8, NEAR_SLOT, 0x000, be_near_slot_end; 
			find_next_slot:;
			
				mov 	1000, CONVENIENCE_2;
				jmp 	inc_slot_number;
				
			synchronization_timer_slot:;
				jg	SINC_SLOT_2, SPR_TSF_WORD2, be_near_slot_end;		/* if () */
				jl	SINC_SLOT_2, SPR_TSF_WORD2, inc_slot_number;		/* if () */
		
				jg	SINC_SLOT_1, SPR_TSF_WORD1, be_near_slot_end;		/* if () */		
				jl	SINC_SLOT_1, SPR_TSF_WORD1, inc_slot_number;		/* if () */		

				jge	SINC_SLOT_0, SPR_TSF_WORD0, be_near_slot_end;		/* if () */		
			//	jl	SINC_SLOT_0, SPR_TSF_WORD0, inc_slot_number;		/* if () */		

			inc_slot_number:;	
				add.	SINC_SLOT_0, CONVENIENCE_2, SINC_SLOT_0;			//2200 us slot time duration
				addc.	SINC_SLOT_1, 0x000, SINC_SLOT_1;
				addc	SINC_SLOT_2, 0x000, SINC_SLOT_2;					
				jmp	synchronization_timer_slot;

			be_near_slot_end:;
				orx 	0, 8, 0xFFFF, NEAR_SLOT, NEAR_SLOT;
				
				jl	SPR_TSF_WORD2, SINC_SLOT_2, check_slot_time_not_detected;		/* if () */		
				jg	SPR_TSF_WORD2, SINC_SLOT_2, find_next_slot;			/* if () */		

				jl	SPR_TSF_WORD1, SINC_SLOT_1, check_slot_time_not_detected;		/* if () */		
				jg	SPR_TSF_WORD1, SINC_SLOT_1, find_next_slot;			/* if () */		

				jl	SPR_TSF_WORD0, SINC_SLOT_0, check_slot_time_not_detected;		/* if () */		
				sub	SPR_TSF_WORD0, SINC_SLOT_0, GP_REG10;	
				mov	0x1E, CONVENIENCE;						
				jg	GP_REG10, CONVENIENCE, find_next_slot;
			
				orx 	0, 8, 0x0000, NEAR_SLOT, NEAR_SLOT; 
			detected_slot_end:;
			next_slot_computed:;
			
		je 	COUNT_SLOT, 0x9, save_slot_9;
		je 	COUNT_SLOT, 0x8, save_slot_8;
		je 	COUNT_SLOT, 0x7, save_slot_7;
		je 	COUNT_SLOT, 0x6, save_slot_6;
		je 	COUNT_SLOT, 0x5, save_slot_5;
		je 	COUNT_SLOT, 0x4, save_slot_4;
		je 	COUNT_SLOT, 0x3, save_slot_3;
		je 	COUNT_SLOT, 0x2, save_slot_2;
		je 	COUNT_SLOT, 0x1, save_slot_1;
		je 	COUNT_SLOT, 0x0, save_slot_0;
		jmp 	check_module;
		
	save_slot_9:
		add 	COUNT_BAD_SLOT_9, COUNT_BAD_SLOT_CURRENT, COUNT_BAD_SLOT_9;
		jmp 	check_module;
	save_slot_8:
		add 	COUNT_BAD_SLOT_8, COUNT_BAD_SLOT_CURRENT, COUNT_BAD_SLOT_8;
		jmp 	check_module;
	save_slot_7:
		add 	COUNT_BAD_SLOT_7, COUNT_BAD_SLOT_CURRENT, COUNT_BAD_SLOT_7;
		jmp 	check_module;
	save_slot_6:
		add 	COUNT_BAD_SLOT_6, COUNT_BAD_SLOT_CURRENT, COUNT_BAD_SLOT_6;
		jmp 	check_module;
	save_slot_5:
		add 	COUNT_BAD_SLOT_5, COUNT_BAD_SLOT_CURRENT, COUNT_BAD_SLOT_5;
		jmp 	check_module;
	save_slot_4:
		add 	COUNT_BAD_SLOT_4, COUNT_BAD_SLOT_CURRENT, COUNT_BAD_SLOT_4;
		jmp 	check_module;
	save_slot_3:
		add 	COUNT_BAD_SLOT_3, COUNT_BAD_SLOT_CURRENT, COUNT_BAD_SLOT_3;
		jmp 	check_module;
	save_slot_2:
		add 	COUNT_BAD_SLOT_2, COUNT_BAD_SLOT_CURRENT, COUNT_BAD_SLOT_2;
		jmp 	check_module;
	save_slot_1:
		add 	COUNT_BAD_SLOT_1, COUNT_BAD_SLOT_CURRENT, COUNT_BAD_SLOT_1;
		jmp 	check_module;
	save_slot_0:
		add 	COUNT_BAD_SLOT_0, COUNT_BAD_SLOT_CURRENT, COUNT_BAD_SLOT_0;
		;//jmp 	check_module;
		

	check_module:;
		add 	COUNT_SLOT, 0x01, COUNT_SLOT;
		jl 	COUNT_SLOT, 10, count_slot_good;
			mov 	0x00, COUNT_SLOT;
		count_slot_good:;
		...
		manage_trace:;
			...
			add	STATE_DEBUG_04, 4, STATE_DEBUG_04;
			;//add	STATE_DEBUG_04, 1, STATE_DEBUG_04;
			mov	REGION_DEBUG_STOP, CONVENIENCE;
			jge	STATE_DEBUG_04, CONVENIENCE, reset_trace;
		write_store_value:;
			mov	STATE_DEBUG_04, SPR_BASE5;
			
			
			rr	COUNT_SLOT, 8, [0x00,off5];
			rr	SPR_TSF_WORD0, 8, [0x01,off5];
			rr	COUNT_BAD_SLOT_CURRENT, 8, [0x02,off5];
			rr	COUNT_GOOD_SLOT_CURRENT, 8, [0x03,off5];
			
			;//mov	COUNT_BAD_SLOT_CURRENT, [0x00,off5];
			;//mov	0xEEEE, [0x01,off5]
			
			
			jmp	trace_state_end;	
		reset_trace:;
			mov	REGION_DEBUG_START,	STATE_DEBUG_04;
			jmp	write_store_value;
		trace_state_end:;
		
		/*debug*/
		
		
		mov 	0x00, COUNT_BAD_SLOT_CURRENT;
		mov 	0x00, COUNT_GOOD_SLOT_CURRENT;
		
	
	check_slot_time_not_detected:;
...
//
	rx_plcp:;
		...
		;//DG2016
		add 	COUNT_GOOD_SLOT_CURRENT, 0x01, COUNT_GOOD_SLOT_CURRENT;
		
	rx_plcp_2:;
		jext	EOI(COND_RX_FCS_GOOD), rx_plcp_2;				
		mov	0, GPHY_SYM_WAR_FLAG;				
...
//
	rx_badplcp:;
		...
		;//DG2016
		;//the goal of this implementation is synchronize the WiFi transmission with the blank LTE frame
		;//we store bad plcp information for the previous 3 slots last slot 1ms, slot before 3ms and slot before before 3ms
		
		add 	COUNT_BAD_SLOT_CURRENT, 0x01, COUNT_BAD_SLOT_CURRENT;
	
	
		;// switch off receiver - maybe doesn't work
